...
export class SearchManager {
    ...
    async doSearches(
        ...
        if (!doMobile && !doDesktop) {
            ...
            await this.runLimitedSearchBonusBeforeMobileClose(
                mobileSession,
                account,
                accountEmail,
                executionContext
    ...
    private async doParallelSearches(
        ...
        try {
            ...
            if (shouldDoMobile) {
                ...
                await this.runLimitedSearchBonusBeforeMobileClose(mobileSession, account, accountEmail, executionContext)
                await this.bot.browser.func.closeBrowser(mobileSession.context, accountEmail)
                mobileContextClosed = true
            ...
            if (shouldDoDesktop) {
                ...
                try {
                    ...
                    this.bot.logger.error(
                        'main',
                        'SEARCH-MANAGER',
                        `Desktop login failed (continuing with mobile only): ${
                            error instanceof Error ? error.message : String(error)
                        }`
                    )
                    if (error instanceof Error && error.stack) {
                        this.bot.logger.debug('main', 'SEARCH-MANAGER', error.stack)
                    }
                }
            } else {
            ...
            if (shouldDoMobile) {
                const mobileResult = settled[resultIndex++]
                if (mobileResult) {
                    if (mobileResult.status === 'fulfilled') {
                        mobilePoints = mobileResult.value
                    } else {
                        const reason =
                            mobileResult.reason instanceof Error
                                ? mobileResult.reason.message
                                : String(mobileResult.reason)
                        this.bot.logger.error('main', 'SEARCH-MANAGER', `Mobile search failed: ${reason}`)
                    }
                }
            }

            if (shouldDoDesktop && desktopSession) {
                const desktopResult = settled[resultIndex]
                if (desktopResult) {
                    if (desktopResult.status === 'fulfilled') {
                        desktopPoints = desktopResult.value
                    } else {
                        const reason =
                            desktopResult.reason instanceof Error
                                ? desktopResult.reason.message
                                : String(desktopResult.reason)
                        this.bot.logger.error('main', 'SEARCH-MANAGER', `Desktop search failed: ${reason}`)
                    }
                }
            } else if (shouldDoDesktop && !desktopSession) {
                this.bot.logger.warn('main', 'SEARCH-MANAGER', 'Desktop search skipped (session not available)')
            }
            ...
            this.bot.logger.error(
                'main',
                'SEARCH-MANAGER',
                `Parallel setup failed: ${error instanceof Error ? error.message : String(error)}`
            )
            if (error instanceof Error && error.stack) {
    ...
    private async runLimitedSearchBonusBeforeMobileClose(
        mobileSession: BrowserSession,
        account: Account,
        accountEmail: string,
        executionContext: any
    ): Promise<void> {
        if (!this.bot.config.workers.doLimitedSearchBonus) return

        try {
            await executionContext.run({ isMobile: true, account }, async () => {
                const data = await this.bot.browser.func.getDashboardData()
                const page = await mobileSession.context.newPage()
                this.bot.logger.info(
                    'main',
                    'SEARCH-MANAGER',
                    'Running 100pt search bonus after daily searches (before mobile close)'
                )
                await this.bot.activities.doLimitedSearchBonuses(data, page)
            ...
            this.bot.logger.warn(
                'main',
                'SEARCH-MANAGER',
                `Limited search bonus failed: ${error instanceof Error ? error.message : String(error)}`
            )
        }
    }

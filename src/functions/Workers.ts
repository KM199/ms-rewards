...
export class Workers {
    ...
    public async doDailySet(data: DashboardData, page: Page) {
        ...
        const afterData = await this.bot.browser.func.getDashboardData()
        const stillOpen =
            afterData.dailySetPromotions[todayKey]?.filter(
                x => !x?.complete && x.pointProgressMax > 0 && x.pointProgress < x.pointProgressMax
            ) ?? []
        ...
        if (stillOpen.length) {
            this.bot.logger.warn(
                this.bot.isMobile,
                'DAILY-SET',
                `${stillOpen.length} daily set item(s) still incomplete: ${stillOpen.map(x => x.title).join(', ')}`
            )
        } else {
    ...
    public async doMorePromotions(data: DashboardData, page: Page) {
        ...
        const activitiesUncompleted: BasePromotion[] =
            morePromotions?.filter(x => {
                ...
                if (x.offerId?.toLowerCase().includes('banner_search')) return false
                ...
                switch (type) {
                    ...
                    case 'quiz': {
                        ...
                        if (activity.pointProgressMax === 10 && destinationUrl.includes('pollscenarioid')) {
                            ...
                            await this.bot.activities.doBrowserQuiz(basePromotion, page)
                            break
                        }
                    ...
                    // UrlReward (includes daily set — complete via reportactivity API, not in-page quiz UI)
                    case 'urlreward': {
                        const basePromotion = activity as BasePromotion
                        ...
                        if (name.includes('exploreonbing')) {
                            ...
                            await this.bot.activities.doUrlRewardWithBrowserFallback(basePromotion, page)
                        }
                        break
                    }
